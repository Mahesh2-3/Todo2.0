...
const Navbar = ({ notifi }) => {
  ...
  const { data: streakData } = useSWR("/api/auth/streak", fetcher, {
  ...
  return (
    <div className="w-full md:h-[10%] h-[7%] px-8 flex items-center bg-[var(--bg-card)] text-[var(--text-main)] justify-between rounded-md relative shadow-dark transition-colors">
      ...
      <div className="flex items-center sm:gap-6 gap-1.5">
        ...
        {/* Streak Display */}
        <div className="flex items-center gap-1 font-bold text-orange-500 sm:text-lg text-xs bg-orange-100 px-2 sm:px-3 py-0.5 sm:py-1 rounded-full shadow-sm">
          🔥 {streakCount}
        </div>
        <button
